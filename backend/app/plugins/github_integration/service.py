"""Github plugin — service layer.

Owns the CRUD for ``github_collection_links`` rows, resolves the
...
from app.plugins.github_integration.models import (
    GithubCollectionLink,
    GithubWebsiteLink,
)
...
_PAT_SETTING_KEY = "github_integration_pat"
...
async def get_config_pat_set(db: AsyncSession) -> bool:
    """True when the global ``github_integration_pat`` is populated."""
...
async def update_config_pat(db: AsyncSession, plaintext: str | None) -> None:
    ...
    if row is None:
        raise DomainValidationError(
            "SETTING_MISSING",
            f"Setting '{_PAT_SETTING_KEY}' missing — did migration 0062 run?",
        )
...
) -> GithubCollectionLink | None:
    return await db.scalar(
        select(GithubCollectionLink).where(
            GithubCollectionLink.collection_id == collection_id,
        )
    )


def _to_response(link: GithubCollectionLink) -> GithubLinkResponse:
...
async def get_link(
    ...
    if link is None:
        raise NotFoundError(
            f"Collection '{slug}' is not linked to a Github repository.",
        )
    return _to_response(link)


async def upsert_link(
    db: AsyncSession, slug: str, data: GithubLinkCreate,
) -> GithubLinkResponse:
    col = await _get_collection_by_slug(db, slug)
    link = await _get_link(db, col.id)
    if link is None:
        link = GithubCollectionLink(collection_id=col.id)
        db.add(link)
...
def _map_forge_error(exc: GitForgeError) -> DomainValidationError:
    ...
    if isinstance(exc, AuthFailed):
        return DomainValidationError(
            "FORGE_AUTH_FAILED",
            "Github rejected the token. Check the PAT and its scopes.",
        )
    ...
    if isinstance(exc, RepoNotFound):
        return DomainValidationError(
            ...
            "Github could not find that owner/name pair or the PAT "
            "cannot see it.",
        )
    ...
    if isinstance(exc, RateLimited):
        return DomainValidationError(
            "FORGE_RATE_LIMITED",
            "Github rate-limited the request. Wait and retry.",
        )
    return DomainValidationError(
...
async def push_collection(
    ...
    *,
    ...
    """Push every TEI file in ``slug`` to the linked Github repo in
    a single commit.

    ``adapter`` and ``transport`` are injectable for tests; production
    code always uses :class:`GithubAdapter` with a real network
    client.
    """
    col = await _get_collection_by_slug(db, slug)
    link = await _get_link(db, col.id)
    ...
    try:
        ...
        logger.warning(
            "github_push_failed", slug=slug, error=str(exc),
        )
...
async def initialize_collection(
    ...
    *,
    ...
    Github repository into an *empty* Aracne2 collection.

    Guards:
      - Collection must exist and be empty (zero documents).
      - A Github link must exist for the collection.
    ...
    if link.initialized_at is not None:
        raise ConflictError(
            ...
            f"Github — push is the only allowed direction from now on.",
        )
    ...
    try:
        ...
        logger.warning(
            "github_initialize_failed", slug=slug, error=str(exc),
        )
        raise _map_forge_error(exc) from exc

    if not bundle.head_sha:
        raise ConflictError(
            f"Github reports no commits on branch '{link.branch}' — "
            f"nothing to import."
        )
    ...
    return GithubInitializeResponse(
...
) -> GithubWebsiteLink | None:
    return await db.scalar(
        select(GithubWebsiteLink).where(
            GithubWebsiteLink.website_id == website_id,
        )
    )


def _website_link_to_response(
    link: GithubWebsiteLink,
) -> GithubWebsiteLinkResponse:
    return GithubWebsiteLinkResponse(
        base_url=link.base_url,
        repo_owner=link.repo_owner,
        repo_name=link.repo_name,
        branch=link.branch,
...
async def get_website_link(
    ...
    if link is None:
        raise NotFoundError(
            f"Website '{slug}' is not linked to a Github repository.",
        )
    return _website_link_to_response(link)


async def upsert_website_link(
    db: AsyncSession, slug: str, data: GithubWebsiteLinkCreate,
) -> GithubWebsiteLinkResponse:
    website = await _get_website_by_slug(db, slug)
    link = await _get_website_link(db, website.id)
    if link is None:
        link = GithubWebsiteLink(website_id=website.id)
        db.add(link)
    ...
    *,
    ...
    if website.build_status != BuildStatus.done:
        raise ConflictError(
            ...
            "and wait for it to finish before pushing to Github.",
        )
    ...
    try:
        ...
        logger.warning(
            "github_website_push_failed", slug=slug, error=str(exc),
        )
        raise _map_forge_error(exc) from exc
    ...
    return GithubWebsitePushResponse(
        sha=result.sha,
        committed_at=link.last_push_at,
        html_url=result.html_url,
