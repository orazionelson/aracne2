"""Gitlab plugin — HTTP router.

Mounted at ``/api/v1/plugins/gitlab``. ACL split:
...
from app.plugins.gitlab_integration import service
...
router = APIRouter(prefix="/plugins/gitlab", tags=["gitlab"])
...
) -> DataResponse[GitlabConfig]:
    return DataResponse(
        data=GitlabConfig(pat_set=await service.get_config_pat_set(db)),
    )


@router.put("/config")
async def update_config(
    body: GitlabConfigUpdate,
    _: Annotated[User, _admin],
    db: Annotated[AsyncSession, Depends(get_async_session)],
) -> DataResponse[GitlabConfig]:
...
async def read_link(
    ...
    db: Annotated[AsyncSession, Depends(get_async_session)],
) -> DataResponse[GitlabLinkResponse]:
...
) -> DataResponse[GitlabPushResponse]:
    try:
...
async def initialize_collection(
    ...
    db: Annotated[AsyncSession, Depends(get_async_session)],
    existdb: Annotated[ExistDBClient, Depends(get_existdb)],
) -> DataResponse[GitlabInitializeResponse]:
    """One-shot import: copy every XML file from the linked Gitlab
...
async def read_website_link(
    ...
    db: Annotated[AsyncSession, Depends(get_async_session)],
) -> DataResponse[GitlabWebsiteLinkResponse]:
...
async def push_website(
    slug: str,
    body: GitlabWebsitePushRequest,
    _: Annotated[User, _designer_plus],
    db: Annotated[AsyncSession, Depends(get_async_session)],
) -> DataResponse[GitlabWebsitePushResponse]:
