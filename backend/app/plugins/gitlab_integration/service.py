"""Gitlab plugin — service layer.

Owns the CRUD for ``gitlab_collection_links`` rows, resolves the
...
from app.plugins.gitlab_integration.models import (
    GitlabCollectionLink,
    GitlabWebsiteLink,
)
from app.plugins.gitlab_integration.schemas import (
    GitlabInitializeResponse,
    GitlabLinkCreate,
    GitlabLinkResponse,
    GitlabPushResponse,
    GitlabWebsiteLinkCreate,
    GitlabWebsiteLinkResponse,
    GitlabWebsitePushResponse,
)
from app.services.settings import get_decrypted_setting

logger = structlog.get_logger()

_PAT_SETTING_KEY = "gitlab_integration_pat"
...
async def get_config_pat_set(db: AsyncSession) -> bool:
    """True when the global ``gitlab_integration_pat`` is populated."""
...
async def update_config_pat(db: AsyncSession, plaintext: str | None) -> None:
    ...
    if row is None:
        raise DomainValidationError(
            "SETTING_MISSING",
            f"Setting '{_PAT_SETTING_KEY}' missing — did migration 0063 run?",
        )
...
) -> GitlabCollectionLink | None:
    return await db.scalar(
        select(GitlabCollectionLink).where(
            GitlabCollectionLink.collection_id == collection_id,
        )
    )


def _to_response(link: GitlabCollectionLink) -> GitlabLinkResponse:
    return GitlabLinkResponse(
...
async def get_link(
    ...
    if link is None:
        raise NotFoundError(
            f"Collection '{slug}' is not linked to a Gitlab repository.",
        )
    return _to_response(link)


async def upsert_link(
    db: AsyncSession, slug: str, data: GitlabLinkCreate,
) -> GitlabLinkResponse:
    col = await _get_collection_by_slug(db, slug)
    link = await _get_link(db, col.id)
    if link is None:
        link = GitlabCollectionLink(collection_id=col.id)
        db.add(link)
...
def _map_forge_error(exc: GitForgeError) -> DomainValidationError:
    ...
    if isinstance(exc, AuthFailed):
        return DomainValidationError(
            "FORGE_AUTH_FAILED",
            "Gitlab rejected the token. Check the PAT and its scopes.",
        )
    ...
    if isinstance(exc, RepoNotFound):
        return DomainValidationError(
            ...
            "Gitlab could not find that owner/name pair or the PAT "
            "cannot see it.",
        )
    ...
    if isinstance(exc, RateLimited):
        return DomainValidationError(
            "FORGE_RATE_LIMITED",
            "Gitlab rate-limited the request. Wait and retry.",
        )
    return DomainValidationError(
...
async def push_collection(
    ...
    *,
    ...
    """Push every TEI file in ``slug`` to the linked Gitlab repo in
    a single commit.

    ``adapter`` and ``transport`` are injectable for tests; production
    code always uses :class:`GitlabAdapter` with a real network
    client.
    """
    col = await _get_collection_by_slug(db, slug)
    link = await _get_link(db, col.id)
    ...
    try:
        ...
        logger.warning(
            "gitlab_push_failed", slug=slug, error=str(exc),
        )
...
async def initialize_collection(
    ...
    *,
    ...
    Gitlab repository into an *empty* Aracne2 collection.

    Guards:
      - Collection must exist and be empty (zero documents).
      - A Gitlab link must exist for the collection.
    ...
    if link.initialized_at is not None:
        raise ConflictError(
            ...
            f"Gitlab — push is the only allowed direction from now on.",
        )
    ...
    try:
        ...
        logger.warning(
            "gitlab_initialize_failed", slug=slug, error=str(exc),
        )
        raise _map_forge_error(exc) from exc

    if not bundle.head_sha:
        raise ConflictError(
            f"Gitlab reports no commits on branch '{link.branch}' — "
            f"nothing to import."
        )
    ...
    return GitlabInitializeResponse(
...
) -> GitlabWebsiteLink | None:
    return await db.scalar(
        select(GitlabWebsiteLink).where(
            GitlabWebsiteLink.website_id == website_id,
        )
    )


def _website_link_to_response(
    link: GitlabWebsiteLink,
) -> GitlabWebsiteLinkResponse:
    return GitlabWebsiteLinkResponse(
        base_url=link.base_url,
        repo_owner=link.repo_owner,
        repo_name=link.repo_name,
        branch=link.branch,
...
async def get_website_link(
    ...
    if link is None:
        raise NotFoundError(
            f"Website '{slug}' is not linked to a Gitlab repository.",
        )
    return _website_link_to_response(link)


async def upsert_website_link(
    db: AsyncSession, slug: str, data: GitlabWebsiteLinkCreate,
) -> GitlabWebsiteLinkResponse:
    website = await _get_website_by_slug(db, slug)
    link = await _get_website_link(db, website.id)
    if link is None:
        link = GitlabWebsiteLink(website_id=website.id)
        db.add(link)
...
async def push_website(
    ...
    *,
    ...
    Gitlab repository in a single commit.
    ...
    if website.build_status != BuildStatus.done:
        raise ConflictError(
            ...
            "and wait for it to finish before pushing to Gitlab.",
        )
    ...
    try:
        ...
        logger.warning(
            "gitlab_website_push_failed", slug=slug, error=str(exc),
        )
        raise _map_forge_error(exc) from exc
    ...
    return GitlabWebsitePushResponse(
        sha=result.sha,
        committed_at=link.last_push_at,
        html_url=result.html_url,
