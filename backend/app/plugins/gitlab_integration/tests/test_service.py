"""Gitlab plugin — service-layer tests.
...
from app.plugins.gitlab_integration.models import GitlabCollectionLink
from app.plugins.gitlab_integration.schemas import GitlabLinkCreate
...
async def test_upsert_link_creates_new_row_and_encrypts_override(
    ...
    row = await db_session.scalar(
        select(GitlabCollectionLink).where(
            GitlabCollectionLink.collection_id == col.id,
        )
    )
...
async def test_push_uses_link_override_when_set(
    ...
    await _mk_setting(db_session, "gitlab_integration_pat", "")
    await service.upsert_link(
        db_session, col.slug,
        GitlabLinkCreate(
            base_url="https://gitlab.com",
...
async def test_push_falls_back_to_global_pat_when_no_override(
    ...
    await _mk_setting(db_session, "gitlab_integration_pat", encrypted)
    await service.upsert_link(
        db_session, col.slug,
        GitlabLinkCreate(
            base_url="https://gitlab.com",
...
async def test_push_success_updates_last_push_columns(
    ...
    adapter = _RecordingAdapter(
        result=CommitResult(
            ...
            html_url="https://gitlab.com/alice/edition/commit/fresh-sha",
        ),
    )
    existdb = _mock_existdb({"a.xml": b"<a/>"})
    resp = await service.push_collection(
