"""GND plugin — editor-side search proxy via lobid.org."""
...
from app.plugins.gnd.schemas import GndHit
from app.plugins.gnd.service import search
from app.schemas.common import DataResponse

router = APIRouter(prefix="/plugins/gnd", tags=["gnd"])


@router.get("/search")
@limiter.limit("30/minute")
async def gnd_search(
    request: Request,
...
) -> DataResponse[list[GndHit]]:
    """Search GND via lobid.org for *q*.

    Returns up to *rows* hits with display label, disambiguating
    detail (biographical dates, profession, or place name), and an
    entity-kind bucket (``person`` / ``corporate`` / ``place`` / …)
    used by the editor panel to validate against the enclosing TEI
    tag. Upstream hiccups degrade to ``data: []``.
    """
