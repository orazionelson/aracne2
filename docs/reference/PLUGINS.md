...
6b. [Bundled non-native plugins](#6b-bundled-non-native-plugins) — see [NON_NATIVE_PLUGINS.md](./NON_NATIVE_PLUGINS.md)
...
are discovered on every startup but start as **inactive** — an Admin activates
each one from `/admin/plugins` and the router mounts on the next backend restart.

The per-plugin catalogue (settings, endpoints, migrations, frontend touchpoints)
