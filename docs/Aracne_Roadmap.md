...
| Ref | Item | One-liner | Status |
|---|---|---|---|
| FUTURE_IDEAS §27 | `policy_pages` plugin | Built-in 12-template engine with platform pre-fill + versioning + IT/EN locales + browser-print PDF + `/policies/<slug>` public render | ✅ Shipped |
| (within §27) | `PolicyManager` capability role | New orthogonal role admin can grant to any User-or-above to delegate policy editing without granting full Admin / EiC privileges | ✅ Shipped |
...
| 2026-05-03 | **FUTURE_IDEAS §24 (`public_navigation` capability) ✅ Shipped**. New `public_navigation` slot in `PluginMeta.ui_descriptor`; `UiConfigResponse.public_nav` exposes active plugins gated by per-plugin `public_link_<name>_enabled` admin toggle (auto-created on activate, default off); three frontend iterators (PublicHeader, PublicHomeSection home_quick_links grid, new PublicFooter swapped into PublicLayout); admin toggle UI in Public Pages → Pagine. |
| 2026-05-03 | **FUTURE_IDEAS §25 (Natural-language search) ✅ Shipped** across Phases NLS-A through NLS-E. New `nl_search` non-native plugin with two Alembic migrations (`nl_search_cache`, `nl_search_budget_day`); `ToolUseProvider` ABC + Ollama + Anthropic adapters; orchestrator runs the LLM tool-use loop against the MCP read subset (`search_entities`, `find_entity_occurrences`, `get_collection`, `list_documents`, `get_document_source`, `tei_to_text`) with citation enforcement that drops any hallucinated `(slug, filename)` pair; SSE endpoint at `POST /api/v1/nl-search/query` with auth / budget / concurrency / cache gates; `NlSearchPublicView.vue` consumer. **Closes Milestone 1 (5 / 5 items shipped).** |
| 2026-05-03 | **DEFERRED §15 (PyJWT migration) ✅ Shipped** — Milestone 2 item 1 of 4. `app/services/auth.py` swapped `from jose import …` for `import jwt`; `python-jose` and its transitive `pyasn1` pin no longer in the image. Closes CVE-2026-30922. |
| 2026-05-03 | **FUTURE_IDEAS §20 (Admin audit log view) ✅ Shipped** — Milestone 2 item 2 of 4. New `/admin/audit-log` UI with structured filters **and** a free-text `q` ILIKE search across `actor_username` / `action` / `target_label`; paginated table sorted newest-first; click-row side panel with pretty-printed JSONB payload; CSV export. Curated `KNOWN_ACTIONS` vocabulary in `services/audit_log.py`. Migration 0078 adds three composite indexes for the predictable filter shapes. CTS R7 reinforced (audit-log presentation surface). |
| 2026-05-03 | **CTS R7 fixity layer ✅ Shipped** — Milestone 2 item 3 of 4 (CTS deliverable, originally implicit; now absorbed into the M2 items table). New `fixity_records` table (Alembic 0079); `services.fixity.record_publication` upserts at every publish; `apscheduler` `fixity_recheck` job with configurable cadence (`daily` / `weekly`, default weekly Sun 03:00 UTC); first `ok → drifted | missing` transition stamps `drifted_at` and emits `fixity.drift_detected` audit row; `/admin/fixity` view with 4 status cards, drift-first table, synchronous "Recheck now" button. Drift is record-only. **CTS R7 transitions 🟡 Partial → ✅ Strong** — heaviest reviewer gap closes. |
| 2026-05-03 | **DEFERRED §16 (pytest 9 triple bump) ✅ Shipped** — Milestone 2 item 4 of 4. `pytest 8.3 → 9.0`, `pytest-asyncio 0.24 → 1.3`, `pytest-cov 6.0 → 7.1` (+ `coverage 7.6 → 7.13` to satisfy the new pytest-cov floor). Two pytest-asyncio config keys already pinned were 1.x-compatible. Closes CVE-2025-71176. **Closes Milestone 2 (4 / 4 items shipped).** |
| 2026-05-03 | **FUTURE_IDEAS §27 (`policy_pages` plugin) ✅ Shipped** across Phases PP-A through PP-I. Two Alembic migrations: `policy_pages` + `policy_page_versions` (0080) and the capability-role schema upgrade (0081 — `kind` + `singleton` columns on `roles`, `PolicyManager` enum value). Declarative `PolicyTemplate` / `Field` engine + 12 built-in templates (mission, privacy_dpia, storage_policy, continuity_plan, preservation_plan, appraisal_policy, incident_response, citation_guide, editorial_board, funding_staffing, expert_directory, cts_self_assessment); 10 platform-introspection helpers feeding `Field(kind="platform")` for auto-refreshing values at render time. Save / Publish / Unpublish split (no review workflow inside the platform). PolicyManager singleton capability role with transactional transfer; new `require_capability` middleware. Admin master/detail editor at `/admin/policies` with IT/EN tabs, version history, and the PolicyManager card; public index at `/policies` and per-policy render at `/policies/<slug>` (Markdown → HTML via markdown-it). PDF via browser-print + `@media print` stylesheet — server-side weasyprint deferred to FUTURE_IDEAS §29. Plugin advertises `public_navigation` so §24's footer iterator can surface a single "Policies" link. **Closes Milestone 3 (1 / 1 items shipped).** |
| 2026-05-03 | **GDPR posture rework ✅ Closed** — the M1 residual deliverable for CTS R4. Replaces the B2C-style hard-delete pattern with the editor-appropriate mediated flow. New Alembic 0082 (`gdpr_requests` table); `services/gdpr.py` with rich `export_personal_data` (art. 15) + `submit_anonymise_request` + Admin-side `anonymise_user_metadata` / `reject_anonymise_request`; new `/admin/gdpr/*` router. The `DELETE /users/me` self-service endpoint was *removed* — anonymisation is now mediated through Admin review under whatever institutional / legal process applies (legal foundation: art. 17.3.d). Documented in [`GDPR_POSTURE.md`](reference/GDPR_POSTURE.md) + the user-facing help page. **R4 transitions 🟡 Partial → ✅ Strong**; CTS counts now **16 ✅ / 0 🟡 / 0 ❌**. |
