...
/**
 * Codeberg Integration — plugin config page.
 *
    ...
    <div class="space-y-4 rounded border border-gray-200 bg-white p-5 text-sm dark:border-gray-700 dark:bg-gray-800">
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">
          {{ t("codeberg.field_global_pat") }}
        </label>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          {{ store.config?.pat_set
            ? t("codeberg.field_pat_set")
            : t("codeberg.field_pat_unset") }}
        </p>
        <input
          v-model="newPat"
          type="password"
          ...
          :placeholder="t('codeberg.field_pat_placeholder')"
        />
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          {{ t("codeberg.field_pat_hint") }}
          <a href="https://codeberg.org/user/settings/applications" target="_blank" rel="noopener" class="text-indigo-600 hover:underline dark:text-indigo-400">
            codeberg.org/user/settings/applications
          </a>
          — {{ t("codeberg.field_pat_scope_hint") }}
        </p>
      </div>
