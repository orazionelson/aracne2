...
/**
 * GND (lobid.org) lookup panel. Writes @ref on <persName> / <placeName>
 * / <orgName> — the three buckets lobid surfaces for person /
 * place / corporate entries.
 */

import { ref, watch, onMounted } from "vue";
import { useI18n } from "vue-i18n";
import { useDebounceFn } from "@vueuse/core";
import { useGndStore, type GndHit } from "@/stores/gnd";
...
const { t } = useI18n();
const gnd = useGndStore();

const query = ref(props.initialQuery?.trim() ?? "");
const results = ref<GndHit[]>([]);
const error = ref<string | null>(null);
const lastApplied = ref<{ gndId: string; tagName: string } | null>(null);
...
async function runSearch(): Promise<void> {
  ...
  error.value = null;
  try {
    results.value = await gnd.search(q);
  } catch (err) {
...
function applyHit(hit: GndHit): void {
  applyError.value = null;
  const outcome = props.onApply(hit.uri);
  if (outcome.ok) {
    lastApplied.value = { gndId: hit.gnd_id, tagName: outcome.tagName };
    return;
  }
  applyError.value =
    outcome.reason === "no_enclosing_tag"
      ? t("gnd.apply_error_no_enclosing_tag")
      : t("gnd.apply_error_not_entity_tag", { tag: outcome.tagName });
}

function kindBadge(kind: GndHit["kind"]): string {
  const map: Record<GndHit["kind"], string> = {
    ...
    work: "bg-sky-50 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300",
    subject: "bg-violet-50 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300",
  ...
  <div class="flex h-full flex-col overflow-hidden bg-white dark:bg-gray-900">
    <div class="flex flex-shrink-0 items-center justify-between border-b border-gray-200 px-3 py-2 dark:border-gray-700">
      <div class="flex items-center gap-2">
        <svg class="h-4 w-4 text-[#1f4e79]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <rect x="3" y="5" width="18" height="14" rx="1" />
          <path d="M7 9h10M7 13h10M7 17h7" />
        </svg>
        <span class="text-sm font-semibold text-gray-700 dark:text-gray-200">{{ t("gnd.panel_title") }}</span>
      </div>
    ...
    <div class="flex flex-shrink-0 flex-col gap-2 border-b border-gray-200 px-3 py-2 dark:border-gray-700">
      <div class="flex items-center gap-2">
        <input
          v-model="query"
          type="text"
          :placeholder="t('gnd.search_placeholder')"
      ...
      <p v-if="gnd.isSearching" class="text-xs text-gray-400 animate-pulse">{{ t("common.loading") }}</p>
    </div>
    ...
    <div v-if="lastApplied || applyError" class="flex-shrink-0 border-b border-gray-200 px-3 py-2 text-xs dark:border-gray-700">
      ...
      <p v-else-if="lastApplied" class="text-green-700 dark:text-green-400">
        {{ t("gnd.applied_success", { gnd: lastApplied.gndId, tag: lastApplied.tagName }) }}
      </p>
    </div>

    <div class="min-h-0 flex-1 overflow-y-auto">
      <p v-if="error" class="px-3 py-3 text-sm text-red-600 dark:text-red-400">{{ error }}</p>
      <p v-else-if="!gnd.isSearching && query.trim().length < 2" class="px-3 py-3 text-xs text-gray-400">{{ t("gnd.idle_hint") }}</p>
      <p v-else-if="!gnd.isSearching && results.length === 0" class="px-3 py-3 text-xs text-gray-400">{{ t("gnd.no_results") }}</p>
      <ul v-else class="divide-y divide-gray-100 dark:divide-gray-700">
        <li v-for="hit in results" :key="hit.gnd_id" class="px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-800">
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <p class="mt-0.5 font-mono text-[11px] text-gray-400">
                GND {{ hit.gnd_id }} · <a :href="hit.uri" target="_blank" rel="noopener" class="hover:underline">d-nb.info</a>
              </p>
            </div>
            <button type="button" class="shrink-0 rounded bg-indigo-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-indigo-700" @click="applyHit(hit)">{{ t("gnd.apply") }}</button>
          </div>
        </li>
      </ul>
    </div>
