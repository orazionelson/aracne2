...
/**
 * "Deposit to Codeberg" section for a website.
 *
 * Mounted inside WebsiteEditView's ``deposit`` tab when the
 * ``codeberg_integration`` plugin is active. Manages the per-website
 * Codeberg link (repo owner/name, branch, optional PAT override)
...
import {
  useCodebergStore,
  type CodebergWebsiteLink,
  type CodebergWebsitePushResponse,
...
const link = ref<CodebergWebsiteLink | null>(null);
const pushResult = ref<CodebergWebsitePushResponse | null>(null);
...
const whyCannotPush = computed<string | null>(() => {
  ...
  if (props.website.rendering_mode === "DYNAMIC") {
    return t("codeberg.website_cannot_push_dynamic");
  }
  if (props.website.build_status !== "done") {
    return t("codeberg.website_cannot_push_not_built");
  }
  return null;
});
  ...
  <section class="rounded border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
    <div class="mb-3 flex items-start justify-between">
      <div>
        <h2 class="text-sm font-semibold text-gray-800 dark:text-gray-100">
          {{ t("codeberg.website_section_title") }}
        </h2>
    ...
    <div v-else-if="editing" class="space-y-3">
      ...
      <input
        ...
        :placeholder="link?.pat_override_set ? t('codeberg.override_replace_hint') : t('codeberg.field_pat_placeholder')"
      />
    ...
    <div v-else-if="link" class="space-y-3 text-sm">
      ...
      <p class="text-xs text-gray-500 dark:text-gray-400">
        <span v-if="link.last_push_sha">
          ...
          <span v-if="link.last_push_file_count"> · {{ t("codeberg.file_count", { n: link.last_push_file_count }) }}</span>
        </span>
      ...
      <div class="flex gap-2">
        <button
          ...
          @click="push"
        >
          {{ store.isPushingWebsite ? t("common.loading") : t("codeberg.push_website_btn") }}
        </button>
        <button
          type="button"
          class="rounded border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-700"
