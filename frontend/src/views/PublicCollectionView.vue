    ...
    <main class="mx-auto max-w-4xl px-4 py-10">
      ...
      <template v-else-if="collection">
        ...
        <div class="mb-8">
          ...
          <p v-if="collection.author" class="mt-1 text-gray-500 italic">{{ collection.author }}</p>
          <p v-if="collection.description" class="mt-3 text-gray-600">{{ collection.description }}</p>
          <div class="mt-3 flex flex-wrap gap-4 text-xs text-gray-400">
            <span v-if="collection.publisher">{{ collection.publisher }}</span>
            <span v-if="collection.pub_year">{{ collection.pub_year }}</span>
          </div>
        </div>
        ...
        <ul v-else class="divide-y divide-gray-200 rounded-xl border border-gray-200 bg-white shadow-sm">
          <li
