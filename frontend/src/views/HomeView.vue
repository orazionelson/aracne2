  ...
  <div v-if="auth.isAuthenticated" class="p-6 max-w-5xl mx-auto">
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8" :class="{ 'sm:grid-cols-5': dashboard.usersTotal !== null }">
        <button
          class="bg-white border border-gray-200 rounded-lg p-4 text-left hover:shadow-md transition-shadow"
          @click="goToCollections()"
        >
          <div class="text-2xl font-bold text-indigo-700">{{ dashboard.collectionsTotal }}</div>
          <div class="text-sm text-gray-500 mt-1">{{ t("home.stat_collections") }}</div>
        </button>
        <button
          class="bg-white border border-gray-200 rounded-lg p-4 text-left hover:shadow-md transition-shadow"
        ...
        >
          ...
          <div class="text-sm text-gray-500 mt-1">{{ t("home.stat_draft") }}</div>
        </button>
        <button
          class="bg-white border border-gray-200 rounded-lg p-4 text-left hover:shadow-md transition-shadow"
        ...
        >
          ...
          <div class="text-sm text-gray-500 mt-1">{{ t("home.stat_review") }}</div>
        </button>
        <button
          class="bg-white border border-gray-200 rounded-lg p-4 text-left hover:shadow-md transition-shadow"
        ...
        >
          ...
          <div class="text-sm text-gray-500 mt-1">{{ t("home.stat_published") }}</div>
        </button>
        <button
        ...
        >
          ...
          <div class="text-sm text-gray-500 mt-1">{{ t("home.stat_users") }}</div>
        </button>
      </div>
      ...
      <div class="mb-8">
        ...
        <div class="flex flex-wrap gap-3">
          <button
            ...
            :key="sc.routeName"
            class="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-700 transition-colors"
      ...
      <div class="mb-8">
        ...
        <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
          <p v-if="dashboard.recentCollections.length === 0" class="text-sm text-gray-400 p-4">
            {{ t("home.recent_empty") }}
          </p>
          <table v-else class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                <th class="text-left px-4 py-2 font-medium text-gray-500">{{ t("collections.title") }}</th>
                <th class="text-left px-4 py-2 font-medium text-gray-500">{{ t("home.col_status") }}</th>
                <th class="text-left px-4 py-2 font-medium text-gray-500">{{ t("home.col_created") }}</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                <td class="px-4 py-2 font-medium text-gray-800">{{ col.title }}</td>
                <td class="px-4 py-2">
                  <span
