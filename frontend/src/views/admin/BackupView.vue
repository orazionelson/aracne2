...
<template>
  <div class="mx-auto max-w-5xl px-4 py-8">
    <h1 class="mb-1 text-2xl font-bold text-gray-900">{{ t("backup.title") }}</h1>
    <p class="mb-6 text-sm text-gray-500">{{ t("backup.subtitle") }}</p>
    ...
    <div class="mb-8 rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <input
          ...
          class="flex-1 rounded-md border border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
        />
        <button
    ...
    <!-- Jobs table -->
    <div v-if="backupStore.jobs.length === 0 && !backupStore.isLoading" class="text-sm text-gray-500">
      {{ t("backup.empty") }}
    </div>
    ...
    <div v-else class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
      <table class="min-w-full divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-50">
          <tr>
            <th class="px-4 py-3 text-left font-medium text-gray-500">{{ t("backup.col_label") }}</th>
            <th class="hidden px-4 py-3 text-left font-medium text-gray-500 md:table-cell">{{ t("backup.col_scopes") }}</th>
            <th class="px-4 py-3 text-left font-medium text-gray-500">{{ t("backup.col_status") }}</th>
            <th class="hidden px-4 py-3 text-left font-medium text-gray-500 lg:table-cell">{{ t("backup.col_started") }}</th>
            <th class="hidden px-4 py-3 text-left font-medium text-gray-500 lg:table-cell">{{ t("backup.col_size") }}</th>
            <th class="px-4 py-3 text-right font-medium text-gray-500">{{ t("backup.col_actions") }}</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
          ...
          >
            ...
            <td class="px-4 py-3 text-gray-900">
              ...
              <p v-if="job.error" class="mt-0.5 text-xs text-red-600">{{ job.error }}</p>
            </td>
            ...
            <td class="hidden px-4 py-3 text-gray-600 md:table-cell">
              <span
                ...
                :key="scope"
                class="mr-1 inline-block rounded bg-gray-100 px-1.5 py-0.5 text-xs"
              >
            ...
            <td class="px-4 py-3 text-right">
              <div class="flex items-center justify-end gap-2">
                <a
                  ...
                  class="inline-flex items-center gap-1 rounded px-2 py-1 text-xs text-indigo-600 hover:bg-indigo-50"
                  download
                >
